...
- 관리자는 전체 채팅방을 조회합니다.
- 관리자 전체 채팅방 목록 조회는 상담 현황 모니터링용 읽기 권한입니다.
- 다른 관리자가 담당 중인 채팅방도 목록에는 표시될 수 있지만, 단건 조회, 메시지 조회, 실시간 구독, 답장은 담당자 접근 정책을 따릅니다.
...
- 이 API는 상담 현황 모니터링용 읽기 API입니다.
- 담당자가 다른 관리자에게 배정된 채팅방의 메시지도 모니터링 목적으로 포함될 수 있습니다.
- 단건 채팅방 조회, 채팅방별 메시지 조회, 실시간 구독, 메시지 발송은 기존 담당자 접근 정책을 따르므로 다른 관리자가 담당 중인 채팅방에는 접근하거나 답장할 수 없습니다.
