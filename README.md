...
- **결제 상태 동기화**: 현재 결제 상태 동기화는 사용자의 결제 확정 요청(`/api/payments/confirm`)과 PortOne 웹훅(`/api/payments/webhook`)에 의존합니다. 따라서 브라우저 종료나 네트워크 오류로 두 경로 모두 서버에 도달하지 못하면, 주문과 결제가 `READY`, `PAYMENT_PENDING`, `CANCEL_REQUESTED` 등의 상태로 남을 수 있습니다. 이 경우 결제 상태가 자동으로 갱신되지 않거나 재고와 쿠폰이 계속 선점될 수 있습니다. 현재는 이런 상태를 별도로 감지·복구하는 스케줄러가 없어, 추후 장시간 처리되지 않은 주문과 결제를 주기적으로 재확인하고 복구하는 배치를 추가할 예정입니다.
