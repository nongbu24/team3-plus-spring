...
캐싱 · 선착순 쿠폰 발급 동시성 제어 · 실시간 채팅 · PG 결제 연동/멱등성 처리를 갖춘 Spring Boot 이커머스 백엔드
...
Spring Boot 기반 전자제품 판매 이커머스 백엔드로, 대용량 트래픽에서도 안전한 선착순 쿠폰 발급, Redis 기반 쿠폰 목록 캐싱, PortOne 결제 확정 멱등성 처리, WebSocket 기반 실시간 1:1 상담 채팅을 구현하는 데 초점을 맞춘 프로젝트입니다.
...
| 상품 | 상품 목록/상세 조회, 조회수 기반 인기 상품 조회 |
| 장바구니 | 상품 담기, 수량 변경, 삭제, 내 장바구니 조회 |
| 주문/결제 | 바로 주문/장바구니 주문 생성, PortOne 결제 연동, 결제 전 주문 취소 |
| 쿠폰 | 쿠폰 이벤트 등록/캐시 조회, 선착순 발급, 보유 쿠폰 조회 |
...
### 쿠폰 이벤트 목록 캐싱
발급 중인 쿠폰 이벤트 목록을 조회할 수 있습니다.

`GET /api/coupon-events`는 Spring `@Cacheable`로 Redis에 페이지 단위(`page + size`)로 캐시됩니다. TTL은 30초로 짧게 설정해 목록이 오래 stale 되지 않도록 하면서도, 반복 조회가 몰리는 구간에서 DB 부하를 줄입니다. 쿠폰이 새로 발급되어 `issuedQuantity`가 바뀌면 `@CacheEvict`로 캐시를 즉시 무효화해, 캐시된 발급 수량이 실제 값과 어긋나지 않도록 했습니다.
...
결제 승인 검증(`/api/payments/confirm`)은 같은 요청이 중복으로 들어와도 최종 상태가 같도록 멱등하게 처리됩니다. 서버가 계산한 결제 금액과 PortOne 승인 금액이 다르면 주문을 취소하고 재고·쿠폰을 복구한 뒤 PortOne 결제 취소를 요청하며, 취소 처리 중 상태(`CANCEL_REQUESTED`)를 별도로 두어 같은 요청이 취소를 중복 호출하지 않도록 했습니다. 결제 시작과 주문 취소처럼 같은 주문을 동시에 건드릴 수 있는 요청은 비관적 락으로 먼저 획득한 요청만 처리되도록 했습니다.
...
- **Payment**: 결제 승인 결과를 저장하며, Order와 1:1로 연결된다.
...
| GET | `/api/coupon-events` | 쿠폰 이벤트 목록 조회(Redis 캐시) |
...
- **쿠폰 동시성 제어**: 선착순 쿠폰 발급의 동시성 제어는 비관적 락 → 분산 락(Lettuce 기반 스핀 락) → 분산 락(Redisson 기반 공정 락) → Redis 원자적 `DECR` 카운터 순서로 방식을 바꿔가며 구현했습니다. "동시성을 막는다"는 같은 목표 아래에도 보호 범위, 대기 방식, 순서 보장 여부가 도구마다 전혀 다르다는 것을 직접 부딪혀가며 배웠고, "분산 락을 적용했다"는 결과보다 "왜 이 방식을 선택했고 무엇을 포기했는지"를 설명할 수 있어야 한다는 피드백이 가장 크게 남았습니다. 다만 지금 구조도 한계가 없는 것은 아닙니다. Redis `DECR`로 재고 차감 자체는 원자적으로 처리되지만, 그 다음 DB의 `issuedQuantity` 증가와 `UserCoupon` 저장은 여전히 요청과 동기적으로 처리됩니다. 트래픽이 지금보다 훨씬 커져 순간적으로 재고를 통과한 요청이 몰리면 같은 쿠폰 이벤트 row에 DB 경합이 발생할 수 있습니다. 다음 대용량 트래픽 처리 프로젝트에서 기회가 된다면, 성공한 요청만 큐에 적재하고 DB 반영은 비동기 워커가 처리하도록 분리하는 방식을 직접 설계해보고 싶습니다.
- **결제 상태 동기화**: 현재 결제 상태 동기화는 사용자의 결제 확정 요청(`/api/payments/confirm`)에만 의존합니다. PortOne 웹훅 수신은 아직 구현하지 않아서, 브라우저 종료나 네트워크 오류로 confirm 요청이 서버에 도달하지 못하면 주문과 결제가 `READY`, `PAYMENT_PENDING`, `CANCEL_REQUESTED` 등의 상태로 남아도 이를 감지·복구할 방법이 없습니다. 이 경우 재고와 쿠폰이 계속 선점될 수 있습니다. 추후 PortOne 웹훅을 연동해 결제 상태를 서버 쪽에서도 감지하고, 장시간 처리되지 않은 주문과 결제를 스케줄러로 주기적으로 재확인·복구하도록 개선할 예정입니다.

이 외에 도메인별(인증/회원/상품/카테고리/장바구니/주문/쿠폰/결제/채팅/포인트/공통) 기능 한계와 우선순위별 해결 과제는 [docs/LIMITATIONS.md](docs/LIMITATIONS.md)에 정리되어 있습니다.
