...
선착순 쿠폰 발급의 동시성 제어는 비관적 락 → 분산 락(Lettuce 기반 스핀 락) → 분산 락(Redisson 기반 공정 락) → Redis 원자적 `DECR` 카운터 순서로 방식을 바꿔가며 구현했습니다. "동시성을 막는다"는 같은 목표 아래에도 보호 범위, 대기 방식, 순서 보장 여부가 도구마다 전혀 다르다는 것을 직접 부딪혀가며 배웠고, "분산 락을 적용했다"는 결과보다 "왜 이 방식을 선택했고 무엇을 포기했는지"를 설명할 수 있어야 한다는 피드백이 가장 크게 남았습니다.

다만 지금 구조도 한계가 없는 것은 아닙니다. Redis `DECR`로 재고 차감 자체는 원자적으로 처리되지만, 그 다음 DB의 `issuedQuantity` 증가와 `UserCoupon` 저장은 여전히 요청과 동기적으로 처리됩니다. 트래픽이 지금보다 훨씬 커져 순간적으로 재고를 통과한 요청이 몰리면 같은 쿠폰 이벤트 row에 DB 경합이 발생할 수 있습니다. 다음 대용량 트래픽 처리 프로젝트에서 기회가 된다면, 성공한 요청만 큐에 적재하고 DB 반영은 비동기 워커가 처리하도록 분리하는 방식을 직접 설계해보고 싶습니다.

## 결제 도메인 참고사항

현재 결제 상태 동기화는 사용자의 결제 확정 요청(`/api/payments/confirm`)과 PortOne 웹훅(`/api/payments/webhook`)에 의존합니다. 따라서 브라우저 종료나 네트워크 오류로 두 경로 모두 서버에 도달하지 못하면, 주문과 결제가 `READY`, `PAYMENT_PENDING`, `CANCEL_REQUESTED` 등의 상태로 남을 수 있습니다.

이 경우 결제 상태가 자동으로 갱신되지 않거나 재고와 쿠폰이 계속 선점될 수 있습니다. 현재는 이런 상태를 별도로 감지·복구하는 스케줄러가 없어, 추후 장시간 처리되지 않은 주문과 결제를 주기적으로 재확인하고 복구하는 배치를 추가할 예정입니다.
