...
사용자가 "리뷰해줘", "코드 리뷰해줘"처럼 현재 브랜치 리뷰를 요청하면 사용한다.

1. `git status --short`, `git diff --stat`, `git diff --name-only`로 변경 범위를 확인한다.
2. 변경 파일과 직접 관련된 코드, 문서, reference만 읽는다.
3. 위험도 높은 문제부터 Findings에 작성한다.
4. 수정 코드는 필요한 경우에만 짧게 예시로 제시한다.

### 2. 특정 파일 리뷰 모드

사용자가 특정 파일, 클래스, 메서드를 지정하면 사용한다.

1. 요청된 파일을 먼저 읽는다.
2. 직접 연결된 코드와 문서만 추가로 확인한다.
3. 요청 범위를 벗어난 리팩토링 제안은 하지 않는다.

### 3. 문서 정합성 리뷰 모드

사용자가 "문서랑 맞는지 봐줘", "API 문서랑 비교해줘"처럼 요청하면 사용한다.

1. 변경된 Controller, DTO, Entity, API 문서를 기준으로 확인 범위를 좁힌다.
2. 필요한 `docs/api/{domain}.md`, `docs/ERD.md`, 관련 convention 문서만 읽는다.
3. 코드와 문서가 다른 부분을 Findings로 명확히 작성한다.
...
- 리뷰 요청만 받았는데 코드를 직접 수정하지 않는다.
- 전체 구현 코드나 대규모 리팩토링 패치를 작성하지 않는다.
- 변경 파일과 관련 없는 문서를 예방 차원에서 읽지 않는다.
- 프로젝트 범위를 넘는 과한 아키텍처를 필수처럼 제안하지 않는다.
- 확인하지 않은 문제를 단정적으로 Findings에 쓰지 않는다.
